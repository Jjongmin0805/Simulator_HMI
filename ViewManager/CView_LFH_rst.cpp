...
void CView_LFH_rst::Draw_Right_Graph(CDC* pDC, Graphics &graphics)
{
	...
	Gdiplus::RectF  _rect_Y_Split[11];
	...
	int nCount2 = theAppDataMng->GetTableMaxCount(_T("spf_dyn_spfo"));// edited by smcho
	double dValue_tmp[3];// edited by smcho

	double ini_max = -10000 , ini_min = 10000;// edited by smcho
	for (int i = 1; i <= nCount2; i++)// edited by smcho
	{
		dValue_tmp[0] = GETVALUE(double, ("spf_dyn_spfo"), ("spfo_gloadp"), i);// edited by smcho
		dValue_tmp[1] = GETVALUE(double, ("spf_dyn_spfo"), ("spfo_genp"), i);// edited by smcho
		dValue_tmp[2] = GETVALUE(double, ("spf_dyn_spfo"), ("spfo_netp"), i);// edited by smcho
		if (ini_max < dValue_tmp[0])// edited by smcho
			ini_max = dValue_tmp[0];// edited by smcho

		if (ini_max < dValue_tmp[2])// edited by smcho
			ini_max = dValue_tmp[2];// edited by smcho

		if (ini_min > dValue_tmp[0])// edited by smcho
			ini_min = dValue_tmp[0];// edited by smcho

		if (ini_min > dValue_tmp[2])// edited by smcho
			ini_min = dValue_tmp[2];// edited by smcho
	}

	int nstep_kw = 2000; //// edited by smcho
	int nMax = 10000, nMin = -6000;// edited by smcho

	if ((ini_max != -10000) && (ini_min != 10000))// edited by smcho
	{
		nMax = max(ceil(ini_max / nstep_kw) * 2000, 2000);// edited by smcho
		nMin = min(floor(ini_min / nstep_kw) * 2000, -2000);// edited by smcho
	}
	



	int nstep = ceil((nMax - nMin)/nstep_kw); //// edited by smcho
	/////////////////////////////////////////////
	
	double dY_Split = double(rect.Height() / nstep); // edited by smcho
	...
	for (int i = 0; i < nstep; i++) // edited by smcho
	{
		_rect_Y_Split[i] = _rect_Y_Name;

		_rect_Y_Split[i].Y = rect.top + (dY_Split * i) -10;

		strSplit.Format(_T("%d"), nMax - (2000 * i)) ;
