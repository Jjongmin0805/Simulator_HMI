...
void CPNOCtrl_Group::SetSkinInit1()
{
	...
	for (i = 0; i < 3; i++)
	{
		switch (i)
		{
		case 0: { sModulePath = GetModulePath() + _T("\\Skin\\SLD\\SideCtrl\\Group\\Style1\\Left.bmp"); };				break;
		case 1: { sModulePath = GetModulePath() + _T("\\Skin\\SLD\\SideCtrl\\Group\\Style1\\Center.bmp"); };				break;
		case 2: { sModulePath = GetModulePath() + _T("\\Skin\\SLD\\SideCtrl\\Group\\Style1\\Right.bmp"); };				break;
		}
	...
	sModulePath = GetModulePath() + _T("\\Skin\\SLD\\SideCtrl\\Group\\Style1\\caption.bmp");
	if (_waccess(sModulePath, FILEACCESS_READ_ONLY) == -1)						return;
...
void CPNOCtrl_Group::SetSkinInit2()
{
	...
	for( i = 0 ; i < 3 ; i++ )
	{
		switch( i )
		{
		case 0: { sModulePath = GetModulePath() + _T("\\Skin\\SLD\\SideCtrl\\Group\\Style2\\Left.bmp"); };				break;
		case 1: { sModulePath = GetModulePath() + _T("\\Skin\\SLD\\SideCtrl\\Group\\Style2\\Center.bmp"); };				break;
		case 2: { sModulePath = GetModulePath() + _T("\\Skin\\SLD\\SideCtrl\\Group\\Style2\\Right.bmp"); };				break;
		}
	...
	sModulePath = GetModulePath() + _T("\\Skin\\SLD\\SideCtrl\\Group\\Style2\\Seleced_Tab.bmp");
	if( _waccess( sModulePath, FILEACCESS_READ_ONLY ) == -1 )						return ;
	m_hBtmSelectedTab																= (HBITMAP)::LoadImage( ::AfxGetInstanceHandle(), (LPCTSTR)sModulePath, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION | LR_DEFAULTSIZE );
	::GetObject( m_hBtmSelectedTab, sizeof(BITMAP), &( m_hBtmSelectedTabInfor ) );

	sModulePath = GetModulePath() + _T("\\Skin\\SLD\\SideCtrl\\Group\\Style2\\Unseleced_Tab.bmp");
	if( _waccess( sModulePath, FILEACCESS_READ_ONLY ) == -1 )						return ;
	m_hBtmUnSelectedTab															= (HBITMAP)::LoadImage( ::AfxGetInstanceHandle(), (LPCTSTR)sModulePath, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION | LR_DEFAULTSIZE );
	::GetObject( m_hBtmUnSelectedTab, sizeof(BITMAP), &( m_hBtmUnSelectedTabInfor ) );
}
...
void CPNOCtrl_Group::SetSkinInit_Res(int nRes_left, int nRes_Center, int nRes_Right, int nRes_Active, int nRes_Inactive)
{
	/*int				i(0);
	CString			sModulePath(_T(""));
	for (i = 0; i < 3; i++)
	{
		switch (i)
		{
		case 0: { sModulePath = GetModulePath() + _T("\\Skin\\SLD\\SideCtrl\\Group\\Style2\\Left.pno"); };				break;
		case 1: { sModulePath = GetModulePath() + _T("\\Skin\\SLD\\SideCtrl\\Group\\Style2\\Center.bmp"); };				break;
		case 2: { sModulePath = GetModulePath() + _T("\\Skin\\SLD\\SideCtrl\\Group\\Style2\\Right.bmp"); };				break;
		}
	...
	CBitmap bmp[5];
	bmp[0].LoadMappedBitmap(nRes_left);
	bmp[1].LoadMappedBitmap(nRes_Center);
	bmp[2].LoadMappedBitmap(nRes_Right);
	bmp[3].LoadMappedBitmap(nRes_Active);
	bmp[4].LoadMappedBitmap(nRes_Inactive);


	m_hBtmCaptionSkin[0] = (HBITMAP)bmp[0].Detach();
	m_hBtmCaptionSkin[1] = (HBITMAP)bmp[1].Detach();
	m_hBtmCaptionSkin[2] = (HBITMAP)bmp[2].Detach();
	m_hBtmSelectedTab = (HBITMAP)bmp[3].Detach();
	m_hBtmUnSelectedTab = (HBITMAP)bmp[4].Detach();

	::GetObject(m_hBtmCaptionSkin[0], sizeof(BITMAP), &(m_BtmCaptionSkinInfor[0]));
	::GetObject(m_hBtmCaptionSkin[1], sizeof(BITMAP), &(m_BtmCaptionSkinInfor[1]));
	::GetObject(m_hBtmCaptionSkin[2], sizeof(BITMAP), &(m_BtmCaptionSkinInfor[2]));
	::GetObject(m_hBtmSelectedTab, sizeof(BITMAP), &(m_hBtmSelectedTabInfor));
	::GetObject(m_hBtmUnSelectedTab, sizeof(BITMAP), &(m_hBtmUnSelectedTabInfor));
...
void CPNOCtrl_Group::SetSkinOption()
{
	...
	for( i = 0 ; i < 6 ; i++ )
	{
		switch( i )
		{
		case 0 : { sModulePath = GetModulePath() + _T("\\Skin\\SLD\\SideCtrl\\Group\\Option\\1_down.bmp"); };		break;
		case 1 : { sModulePath = GetModulePath() + _T("\\Skin\\SLD\\SideCtrl\\Group\\Option\\1_on.bmp"); };			break;
		case 2 : { sModulePath = GetModulePath() + _T("\\Skin\\SLD\\SideCtrl\\Group\\Option\\1_over.bmp"); };		break;
		case 3 : { sModulePath = GetModulePath() + _T("\\Skin\\SLD\\SideCtrl\\Group\\Option\\2_down.bmp"); };		break;
		case 4 : { sModulePath = GetModulePath() + _T("\\Skin\\SLD\\SideCtrl\\Group\\Option\\2_on.bmp"); };			break;
		case 5 : { sModulePath = GetModulePath() + _T("\\Skin\\SLD\\SideCtrl\\Group\\Option\\2_over.bmp"); };		break;
		}
...
void CPNOCtrl_Group::SetTabView( int nTabIndex, CWnd* pView, int nType)
{
...
void CPNOCtrl_Group::RemoveAllTab()
{
	if (m_vecTabInfor.size() > 0)
	{
		vector<CGroupTab_Infor*>::iterator											_iter;
...
int CPNOCtrl_Group::GetTabCount()
{
	return int(m_vecTabInfor.size());
}

int CPNOCtrl_Group::GetTabType(int nTabIndex)
{
	if (nTabIndex < 0 || nTabIndex >= (int)m_vecTabInfor.size())					return -1;

	CGroupTab_Infor*																pTabInfor;
	pTabInfor = m_vecTabInfor[nTabIndex];

	if (pTabInfor && (pTabInfor->m_pView))
	{
		return pTabInfor->m_nType;
	}

	return -1;
}

CWnd* CPNOCtrl_Group::GetTabWnd(int nTabIndex)
{
	if (nTabIndex < 0 || nTabIndex >= (int)m_vecTabInfor.size())					return NULL;
