...
{
public:
	CGroupTab_Infor() : m_bSelected( FALSE ), m_bMoveSelect( FALSE ), m_szTabName( _T("") ), m_TapSize( 0, 0, 0, 0 ), m_nType(0){ m_pView = NULL; };
...
public:
	...
	void																					SetTabView( int nTabIndex, CWnd* pView, int nType );
	...
	////////////////////////
	void																					SetSkinInit_Res(int nRes_left, int nRes_Center, int nRes_Right, int nRes_Active, int nRes_Inactive );
...
public:
	...
	void																					RemoveAllTab();
	int																						GetTabCount();
	int																						GetTabType(int nIdx);
	CWnd*																					GetTabWnd(int nIdx);
};
