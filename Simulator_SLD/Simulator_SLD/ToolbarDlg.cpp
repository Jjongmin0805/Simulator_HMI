...
void CToolbarDlg::OnPaint()
{
	...
	dc.DrawText(_T("ADMS 계통관리시스템 단선도"), CRect(_rect.left, _rect.top, _rect.right, _rect.top + TOOLBAR_TITLE_HEIGHT), DT_CENTER | DT_SINGLELINE | DT_VCENTER);
...
void CToolbarDlg::LoadIcons(void)
{
	...
	sModulePath.Format(_T("%s\\Skin\\SLD\\Toolbar\\logo.pnoimg"), GetModulePath());
	...
	for (int i = 0; i < IDX_TOOLMENU_CNT; i++)
	{
		sModulePath.Format(_T("%s\\Skin\\SLD\\Toolbar\\%d_Down.pnoimg"), GetModulePath(), i + 1);
		if (_waccess(sModulePath, 06) != -1)
		{
		...
		sModulePath.Format(_T("%s\\Skin\\SLD\\Toolbar\\%d_off.pnoimg"), GetModulePath(), i + 1);
		if (_waccess(sModulePath, 06) != -1)
		{
		...
		sModulePath.Format(_T("%s\\Skin\\SLD\\Toolbar\\%d_On.pnoimg"), GetModulePath(), i + 1);
		if (_waccess(sModulePath, 06) != -1)
		{
		...
		sModulePath.Format(_T("%s\\Skin\\SLD\\Toolbar\\%d_Disable.pnoimg"), GetModulePath(), i + 1);
		if (_waccess(sModulePath, 06) != -1)
		{
	...
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 3; j++)
		{
			sModulePath.Format(_T("%s\\Skin\\SLD\\SystemMenu\\%s"), GetModulePath(), GetIconFileName(IDX_CAPTION_MINIMIZE_ON + (i * 4) + j));
			if (_waccess(sModulePath, 06) != -1)
			{
	...
	for (int i = 0; i < IDX_TOOLMENU_CNT; i++)
	{
		if (m_Icons[i])
		{
			...
			else
			{
				...
				{
					if ((m_bBoundarySet && i == 7) || (m_bShowSecInfo && i == 8) || (m_bShowVirtualDG && i == 9))
					{
						hgdiobj = tmpDC.SelectObject(m_Icons[i][3]);
					}
					else
					{
...
void CToolbarDlg::InitMenuBarCtrl()
{
	...
	rectWindSize.right = 412;
	rectWindSize.right = 0;
