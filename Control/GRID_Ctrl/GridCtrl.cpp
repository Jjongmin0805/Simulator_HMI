...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    ...
    else if( ( nChar == VK_F2 ) || ( m_MouseMode == MOUSE_NOTHING && nChar == VK_RETURN ) )
    {
...
void CGridCtrl::OnEndInPlaceEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    switch (pgvItem->lParam)
    {
		case VK_TAB:
						{
							CString szValue( pgvItem->strText );
							if( m_szOldEditValue != szValue )
							{
								SendParentMsgData( pgvItem->row, pgvItem->col, 1 );
							}
						};
		case VK_DOWN:
		case VK_UP:
...
void CGridCtrl::OnEditCell(int nRow, int nCol, CPoint point, UINT nChar)
{
    ...
    if (SendMessageToParent(nRow, nCol, GVN_BEGINLABELEDIT) >= 0)
    {
        ...
        if (pCell)
		{
			m_szOldEditValue = pCell->GetText();
...
void CGridCtrl::SendParentMsgData( int nRow, int nCol, int nUpdate )
{
	if( !IsWindow( m_hWnd ) )
		return ;

	tagCtrl_EditValue			tagMSGdata;
	tagMSGdata.nRow									= nRow;
	tagMSGdata.nCol									= nCol;
	tagMSGdata.nUpdate								= nUpdate;
	tagMSGdata.nUnitIdx								= GetDlgCtrlID();

	CWnd*						pOwner				= GetOwner();
	if( pOwner && IsWindow( pOwner->m_hWnd ) )
	{
		pOwner->SendMessage( GRIDCTRL_SENDMSGDATA, tagMSGdata.nUnitIdx, (LPARAM)&tagMSGdata );
	}
}