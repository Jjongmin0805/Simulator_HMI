// SkinTabDialog.cpp : implementation file
//

#include "stdafx.h"
#include "SkinTabDialog.h"
...
CSkinTabDialog::CSkinTabDialog( UINT nResourceID, CWnd* pParent /*=NULL*/ )
	: CDialog( nResourceID, pParent )
{
	...
	memset( &m_sizeTab,		 0x00, sizeof(SIZE) );
...
CSkinTabDialog::~CSkinTabDialog()
{	
}


void CSkinTabDialog::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(CSkinTabDialog, CDialog)
	ON_WM_CREATE ()
	ON_WM_DESTROY()	
	ON_WM_NCPAINT()	
	ON_WM_NCACTIVATE()
...
// CSkinTabDialog message handlers

void CSkinTabDialog::OnDestroy() 
{
	CDialog::OnDestroy();
...
BOOL CSkinTabDialog::OnNcActivate(BOOL bActive)
{
...
void CSkinTabDialog::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS* lpncsp)
{
	// TODO: Add your message handler code here and/or call default
	if( GetWindowLong(GetSafeHwnd(), GWL_STYLE) & WS_CAPTION )
	{		
...
void CSkinTabDialog::OnNcMouseMove(UINT nHitTest, CPoint point)
{
	...
	if( GetWindowLong(GetSafeHwnd(), GWL_STYLE) & WS_CAPTION )
	{
		...
		CRect rcRestoreTab, rcMinimize, rcMaximize, rcRestore, rcExit;
		int x = 0, y = 0;
		...
		rcRestoreTab.left = rcMinimize.left - m_sizeTab.cx;
		rcRestoreTab.top = y;
		rcRestoreTab.right = rcRestoreTab.left + m_sizeTab.cx;
		rcRestoreTab.bottom = rcRestoreTab.top + m_sizeTab.cy;

		m_hBmpTab  = g_gbl.GetBitmap( IDX_PNG_BOX_RESTORE_TAB );
		...
		if( rcRestoreTab.PtInRect( point ) )
		{
			m_hBmpTab = g_gbl.GetBitmap( IDX_PNG_BOX_RESTORE_TAB_OVER );
		}		
		else if( rcExit.PtInRect( point ) )
		{
...
void CSkinTabDialog::OnNcLButtonDown(UINT nHitTest, CPoint point)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
	if( GetWindowLong(GetSafeHwnd(), GWL_STYLE) & WS_CAPTION )
	{
		...
		{
			SetCapture();
			m_nClickedBox = CLICKED_BOX_RESTORE_TAB;
			m_hBmpTab = g_gbl.GetBitmap( IDX_PNG_BOX_RESTORE_TAB_DOWN );
...
void CSkinTabDialog::OnNcMouseLeave()
{
...
void CSkinTabDialog::SetBkColor( COLORREF clrBkColor )
{
	m_cfBkMain = clrBkColor;
}


void CSkinTabDialog::DrawBitmap()
{
	if( !( GetWindowLong( GetSafeHwnd(), GWL_STYLE ) & WS_CAPTION ) )
		return;
	...
	// Restore Tab Box 비트맵을 메모리 DC로 전송
	tmpDC.SelectObject( m_hBmpTab );
	x = x - m_sizeTab.cx;
	BufferDC.BitBlt( x, y, m_sizeTab.cx, m_sizeTab.cx, &tmpDC, 0, 0, SRCCOPY );
...
BOOL CSkinTabDialog::OnSetCursor( CWnd* pWnd, UINT nHitTest, UINT message ) 
{
	// TODO: Add your message handler code here and/or call default	
...
BOOL CSkinTabDialog::PreTranslateMessage(MSG* pMsg) 
{
	...
	{	
		if( GetWindowLong(GetSafeHwnd(), GWL_STYLE) & WS_CAPTION )
		{
			CRect rc, rcRestoreTab, rcExit, rcMaximize, rcMinimize;
			GetWindowRect(&rc);	
			...
			if( rcRestoreTab.PtInRect(pMsg->pt) )
			{
				if( m_nClickedBox == CLICKED_BOX_RESTORE_TAB )
				{
					m_nRestoreTab = 1;
					PostMessage( WM_CLOSE, 0, 0 );
				}
			}
			else if( rcExit.PtInRect(pMsg->pt) )
			{
...
void CSkinTabDialog::OnGetMinMaxInfo( MINMAXINFO FAR* lpMMI ) 
{
	// TODO: Add your message handler code here and/or call default		
...
void CSkinTabDialog::OnTimer( UINT_PTR nIDEvent )
{
	CDialog::OnTimer( nIDEvent );
}


int CSkinTabDialog::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_sizeTab   = g_gbl.GetBitmapSize( IDX_PNG_BOX_RESTORE_TAB    );
...
void CSkinTabDialog::OnNcLButtonDblClk(UINT nHitTest, CPoint point)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
