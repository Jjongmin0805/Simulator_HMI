...
namespace JWXml
{
	...
	CRITICAL_SECTION		xmlcs;					//!< Painting중 드로잉 버퍼 업데이트 방지
	void Lock() {::EnterCriticalSection(&xmlcs);}
	void UnLock() {::LeaveCriticalSection(&xmlcs);}
	void InitLock() {::InitializeCriticalSection(&xmlcs);}
	void DeInitLock() {::DeleteCriticalSection(&xmlcs);}
	};
}