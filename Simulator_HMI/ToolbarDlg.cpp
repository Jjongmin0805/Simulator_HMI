...
BEGIN_MESSAGE_MAP(CToolbarDlg, CDialog)
	...
	ON_WM_MOUSEMOVE()
	ON_WM_LBUTTONUP()
	ON_COMMAND(ID_RUNAPPLICATIONSTUDY_APPLICATIONTEST, &CToolbarDlg::OnRunapplicationstudyApplicationtest)
	ON_COMMAND(ID_FILE_OPEN, &CToolbarDlg::OnFileOpen)
	ON_COMMAND(ID_FILE_SAVE, &CToolbarDlg::OnFileSave)
	ON_COMMAND(ID_FILE_SAVE_AS, &CToolbarDlg::OnFileSaveAs)
	ON_COMMAND(ID_SETTING_MTR, &CToolbarDlg::OnSettingMtr)
	ON_COMMAND(ID_SETTING_LINETYPE, &CToolbarDlg::OnSettingLinetype)
	ON_COMMAND(ID_SETTING_RELAY, &CToolbarDlg::OnSettingRelay)
	ON_COMMAND(ID_SETTING_RECLOSER, &CToolbarDlg::OnSettingRecloser)
	ON_COMMAND(ID_SETTING_APP, &CToolbarDlg::OnSettingApp)
	ON_COMMAND(ID_SETTING_USER, &CToolbarDlg::OnSettingUser)
	ON_COMMAND(ID_ADMS_KACIM_CONVERT, &CToolbarDlg::OnAdmsKacimConvert)
	ON_COMMAND(ID_SETTING_LOADFLOW, &CToolbarDlg::OnSettingLoadflow)
	ON_COMMAND(ID_ADMS_SOMAS, &CToolbarDlg::OnAdmsSomas)
	ON_COMMAND(ID_ADMS_SHARE, &CToolbarDlg::OnAdmsShare)
	ON_COMMAND(ID_ADMS_APPLINIT, &CToolbarDlg::OnAdmsApplinit)
	ON_COMMAND(ID_OCB_RELAY_IMPORT, &CToolbarDlg::OnOcbRelayImport)
	ON_COMMAND(ID_MEMOPS, &CToolbarDlg::OnMemops)
	ON_COMMAND(ID_EQUIP_INSERT, &CToolbarDlg::OnEquipInsert)
END_MESSAGE_MAP()
...
void CToolbarDlg::OnPaint()
{
	...
	dc.DrawText( _T("ADMS 계통관리시스템 대시보드"), CRect(_rect.left, _rect.top, _rect.right, _rect.top+TOOLBAR_TITLE_HEIGHT), DT_CENTER | DT_SINGLELINE | DT_VCENTER );
...
void CToolbarDlg::LoadIcons(void)
{
	...
	sModulePath.Format( _T("%s\\Skin\\Toolbar\\HMI\\logo.pnoimg"), GetModulePath() );
	...
	for( int i = 0; i < IDX_TOOLMENU_CNT; i++ )
	{
		sModulePath.Format( _T("%s\\Skin\\Toolbar\\HMI\\%d_Down.pnoimg"), GetModulePath(), i+1 );
		if( _waccess( sModulePath, 06 ) != -1 )
		{
		...
		sModulePath.Format( _T("%s\\Skin\\Toolbar\\HMI\\%d_off.pnoimg"), GetModulePath(), i+1 );
		if( _waccess( sModulePath, 06 ) != -1 )
		{
		...
		sModulePath.Format( _T("%s\\Skin\\Toolbar\\HMI\\%d_On.pnoimg"), GetModulePath(), i+1 );
		if( _waccess( sModulePath, 06 ) != -1 )
		{
		...
		sModulePath.Format(_T("%s\\Skin\\Toolbar\\HMI\\%d_Disable.pnoimg"), GetModulePath(), i + 1);
		if (_waccess(sModulePath, 06) != -1)
		{
...
void CToolbarDlg::DrawToolbar(CDC* pDC)
{
	...
	for( int i = 0; i < IDX_TOOLMENU_CNT; i++ )
	{
		if( m_Icons[i] )
		{
			if( i == m_nSelectIcon )
			{
				hgdiobj = tmpDC.SelectObject( m_Icons[i][2] );
			}
			else
			{
...
BOOL CToolbarDlg::OnChildNotify(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pLResult)
{
...
void CToolbarDlg::OnRunapplicationstudyApplicationtest()
{
	m_pFrameWnd->SendMessage(WM_TOOLBAR_MSG, 2, ID_RUNAPPLICATIONSTUDY_APPLICATIONTEST);
}


void CToolbarDlg::OnFileOpen()
{
	m_pFrameWnd->SendMessage(WM_TOOLBAR_MSG, 2, ID_FILE_OPEN);
}


void CToolbarDlg::OnFileSave()
{
	m_pFrameWnd->SendMessage(WM_TOOLBAR_MSG, 2, ID_FILE_SAVE);
}


void CToolbarDlg::OnFileSaveAs()
{
	m_pFrameWnd->SendMessage(WM_TOOLBAR_MSG, 2, ID_FILE_SAVE_AS);
}


void CToolbarDlg::OnSettingMtr()
{
	m_pFrameWnd->SendMessage(WM_TOOLBAR_MSG, 2, ID_SETTING_MTR);
}


void CToolbarDlg::OnSettingLinetype()
{
	m_pFrameWnd->SendMessage(WM_TOOLBAR_MSG, 2, ID_SETTING_LINETYPE);
}


void CToolbarDlg::OnSettingRelay()
{
	m_pFrameWnd->SendMessage(WM_TOOLBAR_MSG, 2, ID_SETTING_RELAY);
}


void CToolbarDlg::OnSettingRecloser()
{
	m_pFrameWnd->SendMessage(WM_TOOLBAR_MSG, 2, ID_SETTING_RECLOSER);
}


void CToolbarDlg::OnSettingApp()
{
	m_pFrameWnd->SendMessage(WM_TOOLBAR_MSG, 2, ID_SETTING_APP);
}


void CToolbarDlg::OnSettingUser()
{
	m_pFrameWnd->SendMessage(WM_TOOLBAR_MSG, 2, ID_SETTING_USER);
}


void CToolbarDlg::OnAdmsKacimConvert()
{
	m_pFrameWnd->SendMessage(WM_TOOLBAR_MSG, 2, ID_ADMS_KACIM_CONVERT);
}


void CToolbarDlg::OnSettingLoadflow()
{
	m_pFrameWnd->SendMessage(WM_TOOLBAR_MSG, 2, ID_SETTING_LOADFLOW);
}


void CToolbarDlg::OnAdmsSomas()
{
	m_pFrameWnd->SendMessage(WM_TOOLBAR_MSG, 2, ID_ADMS_SOMAS);
}


void CToolbarDlg::OnAdmsShare()
{
	m_pFrameWnd->SendMessage(WM_TOOLBAR_MSG, 2, ID_ADMS_SHARE);
}


void CToolbarDlg::OnAdmsApplinit()
{
	m_pFrameWnd->SendMessage(WM_TOOLBAR_MSG, 2, ID_ADMS_APPLINIT);
}


void CToolbarDlg::OnOcbRelayImport()
{
	m_pFrameWnd->SendMessage(WM_TOOLBAR_MSG, 2, ID_OCB_RELAY_IMPORT);
}


void CToolbarDlg::OnMemops()
{
	m_pFrameWnd->SendMessage(WM_TOOLBAR_MSG, 2, ID_MEMOPS);
}


void CToolbarDlg::OnEquipInsert()
{
	m_pFrameWnd->SendMessage(WM_TOOLBAR_MSG, 2, ID_EQUIP_INSERT);
}
